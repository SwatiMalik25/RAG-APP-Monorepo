...
import { ChatMessage, createMessage } from "../../store/session";
...
export function Chat() {
  ...
  useEffect(() => {
    ...
    botStore.updateBotSession((session: { messages: any[]; }) => {
  ...
  const clearContextIndex =
    (session.clearContextIndex ?? -1) >= 0
      ...
      const clearContext = () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        botStore.updateBotSession((session: any) => {
  ...
  return (
    <div className="flex flex-col relative h-full" key={bot.id}>
      ...
      <ScrollArea
        ...
        ref={scrollRef}
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        onScroll={(e:any) => onChatBodyScroll(e.currentTarget)}
